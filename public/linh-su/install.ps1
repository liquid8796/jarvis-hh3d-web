...
#   • Đường dẫn TUYỆT ĐỐI. Linh sứ tự chạy lúc đăng nhập, mà PATH lúc ấy không giống
...
# --- 1. Dừng linh sứ cũ trước khi ghi đè -------------------------------------
...
# linh sứ mang CÙNG một WORKER_ID, cùng giành job, cùng mở browser trên một máy.
...
foreach ($p in $stale) {
  Write-Host "Dừng linh sứ cũ (PID $($p.ProcessId) $($p.Name))..." -ForegroundColor Yellow
...
Write-Host ("Node " + (& $nodeExe --version) + " — sẵn sàng (riêng của linh sứ)") -ForegroundColor Green
...
Invoke-Tar @("-xzf", $tgz, "-C", $dir) "Bung gói linh sứ"
...
while ($true) {
  # Cắt nhật ký khi quá 5MB. Linh sứ chạy quanh năm, và khi web không với tới được nó
...
# xen vào giữa là linh sứ sống dậy sau khi ta tưởng đã tiễn nó đi.
...
# Thư mục cha chỉ tồn tại vì linh sứ; rỗng rồi thì đừng để lại dấu vết.
...
if (Test-Path "$dir") {
  ...
  Write-Host "Đã gỡ linh sứ túc trực — máy trở lại y như trước khi cài." -ForegroundColor Green
}
