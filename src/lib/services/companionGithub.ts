...
import { isCompanionPathAllowed, isCompanionSourcePath } from "@/lib/validation/companionLanguages";
...
export type RepoSnapshot = { branch: string; head: string | null; tree: string | null; files: Map<string, TreeFile>; context: string; githubId: number; complete?: boolean; todayCommits?: number; language?: string };
export type RepoInfo = { id: number; full_name: string; default_branch: string; description?: string; private: boolean; parent?: { full_name: string }; license?: { spdx_id?: string }; created_at?: string; language?: string | null };
...
function assertDeclaredSourcePaths(paths: readonly string[] | undefined, max = 24): void {
  if (paths && (paths.length > max || new Set(paths.map((path) => path.toLowerCase())).size !== paths.length || paths.some((path) => !safeSourcePath(path) || !isCompanionSourcePath(path, paths)))) throw new Error("Đường dẫn source khai báo không hợp lệ; tài liệu, cấu hình và tài nguyên không phải source.");
}
...
export class CompanionGithub {
  ...
  async snapshot(owner: string, repo: string, contextWindow: number, declaredSourcePaths?: readonly string[]): Promise<RepoSnapshot | null> {
    assertDeclaredSourcePaths(declaredSourcePaths, 256);
    ...
    if (!ref?.object?.sha) return { branch: info.default_branch || "main", head: null, tree: null, files, context: "Empty repository: write the initial working source files.", githubId: info.id, complete: true, todayCommits: 0, ...(info.language ? { language: info.language } : {}) };
    ...
    const context = `Repository: ${info.full_name}\nDescription: ${stripCompanionMarker(info.description ?? "")}\nLanguage: ${info.language ?? "Unknown"}\nRecent commits:\n${history.map((c) => c.commit.message.slice(0, 500)).join("\n")}\nFiles${listing.truncated ? " (partial tree; additions are blocked)" : ""}:\n${Array.from(files.keys()).filter(safeSourcePath).join("\n").slice(0, Math.floor(budget / 4))}\n`;
    const candidates = listing.tree.filter((e) => e.type === "blob" && e.mode !== "120000" && safeSourcePath(e.path) && (e.size ?? Infinity) < 32_000 && (isCompanionSourcePath(e.path, declaredSourcePaths) || /\.(md|json|toml|yaml|yml|txt)$/i.test(e.path)))
    ...
    return { branch: info.default_branch, head: ref.object.sha, tree: commit.tree.sha, files, context, githubId: info.id, complete: !listing.truncated, todayCommits: today.length, ...(info.language ? { language: info.language } : {}) };
  }

  async commit(owner: string, repo: string, snapshot: RepoSnapshot, changes: SourceFile[], message: string, readPaths?: readonly string[], maxCommits = 2, declaredSourcePaths?: readonly string[]): Promise<{ sha: string; pushed: number; partial?: boolean }> {
    assertDeclaredSourcePaths(declaredSourcePaths);
    ...
    if (!selected.some((file) => file.content !== null && isCompanionSourcePath(file.path, declaredSourcePaths))) throw new Error("Commit cần có source thực sự thay đổi; không dùng source giữ nguyên để tạo commit chỉ sửa tài liệu.");
    ...
    let pushed = 0;
    if (!head) {
      const first = selected.find((f) => f.content !== null && isCompanionSourcePath(f.path, declaredSourcePaths)) ?? selected.find((f) => f.content !== null);
  ...
  async create(owner: string, repo: string, description: string): Promise<RepoInfo> {
    ...
    return this.call("POST", path, { name: repo, description: stripCompanionMarker(description).slice(0, 350), private: false, auto_init: false, has_issues: true });
  }
